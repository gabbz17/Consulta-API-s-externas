...
public class InfoService {
    ...
    public WeatherResponseDto getWeather(String city){
        ResponseEntity<WeatherResponseDto> response = restTemplate.getForEntity(String.format("https://api.openweathermap.org/data/2.5/weather?q=%s&appid={APIKEY}&lang=pt_br", city), WeatherResponseDto.class);
        return response.getBody();
    }
    ...
    public InfoResponseDto getInfo(String cep) {
        ...
        ResponseEntity<WeatherResponseDto> response_weather = restTemplate.getForEntity(String.format("https://api.openweathermap.org/data/2.5/weather?q=%s&appid={APIKEY}&lang=pt_br", response_cep.getBody().localidade()), WeatherResponseDto.class);
